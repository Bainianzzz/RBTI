...
function onSubmit() {
  ...
  const txt = freeInput.value
  store.submitAnswer(opt, txt).finally(() => {
...
<template>
  <main class="min-h-dvh bg-gradient-to-b from-amber-50 via-stone-50 to-stone-100 px-5 py-8 text-stone-800">
    <div class="mx-auto w-full max-w-2xl">
      ...
      <div v-else-if="phase === 'answering' && active" :key="events.length">
        <Transition name="fade" appear>
          <article>
            ...
            <div class="mt-6 flex items-center justify-between">
              ...
              <button
                class="inline-flex items-center gap-1.5 rounded-lg bg-stone-900 px-6 py-2.5 text-sm font-medium text-white transition enabled:hover:bg-stone-800 disabled:cursor-not-allowed disabled:opacity-40"
