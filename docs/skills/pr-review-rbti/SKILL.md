...
7. 审核已有 PR 时直接提交 GitHub review；调用 `$pr-review-rbti` 即表示授权本次提交。只有用户明确要求“仅草稿”或审核对象只是本地 diff 时，才只输出报告。
...
每条 finding 都要包含位置、相关代码引用、触发或攻击场景、影响和最小修复方向。代码引用只截取证明问题所需的最小表达式、语句或标识符。优先使用最小必要严重性；不要用“可能”“也许”代替证据。
...
审核已有 PR 时必须提交一次 GitHub review。除非用户明确要求仅生成草稿，否则不要只在对话中返回结果。

- 可定位 finding 必须作为 diff inline comment，并引用触发问题的相关代码。
- 每条 inline comment 只写一个 finding，包含严重性、最小代码引用、场景、影响和建议。
- 跨文件问题或目标行不在 diff 中时放顶层 body，同时写明 `path:line` 并引用相关代码；不要强挂到无关行。
- 顶层 body 只写总体结论、未挂行问题和验证结果，不重复 inline findings。
